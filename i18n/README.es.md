...
Esta configuración la tengo montada en RackNerd. Usa la guía para elegir un servidor que encaje con tu presupuesto y tu red.

**[Mira RackNerd y ayúdame a mantener esta guía](https://my.racknerd.com/aff.php?aff=21220)**
...
Panel opcional: añade [simplify-panel.js](../scripts/simplify-panel.js) al script principal. Para el modo manual, elige un nodo en la lista inferior y después 「切换到手动节点」 arriba. En suscripciones de varios países, aplica [subscription-japan-url-test.js](../scripts/subscription-japan-url-test.js) a cada tarjeta: solo selecciona nodos japoneses de esa suscripción. Las reglas de conexión directa nacional siguen activas en modo de reglas. Las selecciones guardadas pueden prevalecer sobre el valor inicial; una etiqueta Timeout antigua no demuestra por sí sola un fallo. Consulta los [límites](../docs/client-failover.md) y la [configuración de otro ordenador](../docs/family-client-setup.md).
