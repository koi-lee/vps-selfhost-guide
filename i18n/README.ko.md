...
공통 UI 이름은 `上网线路`(경로 선택), `自动切换（推荐）`(자동 전환, 권장)입니다. 자동 순서는 HY2 → CDN → IPv4이며 IPv6는 수동, DIRECT는 마지막입니다. 내부 그룹이 표시되며 검사 간격은 15초, 제한 시간은 5초입니다. 전환은 이후 연결에 적용되므로 이미 실패한 요청은 앱에서 재시도해야 할 수 있습니다.
