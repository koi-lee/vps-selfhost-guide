...
まず上の RackNerd 支援リンクから自分のアカウントを登録し、ログインします。既存アカウントはそのまま利用でき、利用可能な VPS があれば購入は不要です。アカウント登録だけではサーバーは購入されません。プランと支払いを確認し、開通後にデプロイを開始してください。個人情報は本人が入力し、認証情報は公開しないでください。
