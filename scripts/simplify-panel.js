...
function simplifiedMain(config, profileName) {
  var reserved = ['上网方式','自动上网（推荐）','日本上网（自动测速）','直连上网（不经代理）','切换到手动节点','全部节点列表'];
  ...
  if (mainGroup) {
    ...
    mainGroup.proxies.splice(mainGroup.proxies.length - 1, 0, '切换到手动节点');
    ...
    groups.push({name:'切换到手动节点', type:'select', proxies:['全部节点列表'], hidden:true});
    groups.push({name:'全部节点列表', type:'select', proxies:config.proxies.map(function(p) {return p.name;}), hidden:false});
  }
  return config;
}
main = simplifiedMain;
